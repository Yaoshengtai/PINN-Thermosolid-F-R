...
class Monitor2DSpatial:
    ...
    def check(self, approximator, history,epoch):
        ...
        for metric_name, metric_values in history.items():
            if metric_name[:5]=="valid" or metric_name=="train_loss" or metric_name[:6]=='weight':
                continue
            j=j+1
            if j>=3:
                i=i+1
        ...
        u=approximator.__call__(x.requires_grad_(),y.requires_grad_())[:,0]

        u=u.detach().cpu().numpy()
        x=x.detach().cpu().numpy()
        axs[3,1].plot(x,u,label='predict')
        axs[3,1].plot(x,2*x**3-3*x**2+1,label='exact')
        axs[3,1].set_xlabel('r')
        axs[3,1].set_ylabel('temprature')
        axs[3,1].set_title('up_boundary_compare')
        axs[3,1].legend()
        ...
        heatmap=axs[3,2].pcolormesh(xx, yy, uu_di.reshape(xx.shape).T, cmap='rainbow')
        cbar=plt.colorbar(heatmap,ax=axs[3,2],label='Temperature(/K)')
        # 添加轴标签
        # 手动设置 colorbar 的刻度标签
        ...
        axs[3,2].set_title('Heatmap_compare')

        #plt.legend()
        plt.tight_layout()
        plt.savefig(self.args.save_dict+"-image/"+str(epoch)+".png" , dpi=100)
